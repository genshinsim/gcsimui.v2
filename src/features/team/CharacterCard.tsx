...
const slots: SlotKey[] = ["flower", "plume", "sands", "goblet", "circlet"];
...
export default function CharacterCard({ char, index }: CharacterCardProps) {
  ...
  for(let i=0; i<6; i++) {
    consImages[i] = `public/images/avatar/cons/${charURLName}_c${i+1}.png`;
  }
      ...
      arts.push(
        ...
        >
          ...
          <span className="text-center text-xs">
            {statToString(char.artifact[slot].mainStatKey)}
          </span>
        </div>
      );
    } else {
  ...
  return (
    <>
      ...
      <div className="min-h-48 bg-gray-700 shadow rounded-md text-sm flex flex-col p-2 justify-center gap-2">
        ...
        <div className="weapon-parent p-2 bg-gray-800 rounded-md">
          <div className="flex flex-row">
            <div
              className="rounded-md hover:bg-gray-500 cursor-pointer"
            ...
            <div className="flex-grow">
              ...
              >
                {char.weapon.key === "" ? (
                  <div className="font-medium mb-1">Select a weapon first</div>
                ) : (
                  <div>
        ...
        <div className="ml-4 mr-4 w-auto p-2 bg-gray-600 rounded-full flex flex-row justify-center h-18">
          {arts}
        </div>
        ...
        <div className="flex w-full justify-end space-x-2 flex-grow items-end">
          <button
            style={{ maxHeight: "32px" }}
            className="inline-flex items-center bg-red-700 border-0 py-1 px-3 focus:outline-none hover:bg-red-600 rounded text-base mt-4 md:mt-0"
            onClick={deleteChar}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
...
function CharHeader({
  ...
  return (
    <div
      className={
        "character-parent flex flex-row pt-4 pl-4 pr-2 -mt-2 -mr-2 -ml-2 rounded-t-md " +
        charBG(char.element)
      }
    >
      ...
      <div className="w-1/2">
        <img src={char.icon} alt={char.name} className="w-full h-auto " />
      </div>
    </div>
  );
}
